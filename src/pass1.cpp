    ...
    while (opcode != "END" && opcode != "CSECT")
    {
      ...
      if (!checkCommentLine(fileLine))
      {
        ...
        else if (opcode == "EQU")
        {
          ...
          {
            tempOperand = intToStringHex(string_to_decimal(operand), 6);
            relative = 0;
          }
          else
          {
       ...
       {
         writeData = "Use of Control Section detected in line :" + to_string(lineNumber) + ". Control section is not implemented yet. ";
         writeToFile(errorFile, writeData);
         error_flag = true;
