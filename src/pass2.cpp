  ...
  while (opcode != "END")
  {
    while (opcode != "END" && opcode != "CSECT")
    {
      if (!isComment)
      {
        if (OPTAB[getRealOpcode(opcode)].exists == 'y')
        {
          ...
          else if (OPTAB[getRealOpcode(opcode)].format == 2)
          {
            ...
            { // If not two operand i.e. a
              ...
              {
                objectCode = OPTAB[getRealOpcode(opcode)].opcode + intToStringHex(string_to_decimal(operand1), 1) + '0';
              }
            ...
            { // Two operands i.e. a,b
              ...
              {
                objectCode = OPTAB[getRealOpcode(opcode)].opcode + REGTAB[operand1].num + intToStringHex(string_to_decimal(operand2), 1);
              }
        ...
        {
          objectCode = intToStringHex(string_to_decimal(operand), 6);
        }
        else if (opcode == "BASE")
        {
